...
public class DbHelperPbap extends SQLiteOpenHelper {
    ...
    private String TAG = "DbHelperPbap";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        sb2.append(ADDRESS_DETAIL_FIELD[1]);
        sb2.append(") REFERENCES PhoneBookContent(");
        sb2.append(PHONEBOOK_CONTENT_FIELD[0]);
        sb2.append(") );");
        db.execSQL(sb2.toString());
        db.execSQL("CREATE TABLE IF NOT EXISTS EmailDetail (" + EMAIL_DETAIL_FIELD[0] + " INTEGER primary key autoincrement, " + EMAIL_DETAIL_FIELD[1] + " INTEGER, " + EMAIL_DETAIL_FIELD[2] + " INTEGER, " + EMAIL_DETAIL_FIELD[3] + " nvarchar(100),FOREIGN KEY(" + EMAIL_DETAIL_FIELD[1] + ") REFERENCES PhoneBookContent(" + PHONEBOOK_CONTENT_FIELD[0] + ") );");
        StringBuilder sb3 = new StringBuilder();
        sb3.append("CREATE TABLE IF NOT EXISTS PhoneType (");
    ...
    public int insertNumberType(SQLiteDatabase db, ArrayList<HashMap<String, String>> list) {
        ...
        for (int x = 0; x < list.size(); x++) {
            try {
                ...
                Log.d(this.TAG, var8.getMessage());
            }
        }
        return (int) row;
    }
    ...
    public Cursor isEmptyPhoneType(SQLiteDatabase db) {
        return db.rawQuery("select count(*) as amount from PhoneType", null);
    }
    ...
    public void insertPhoneType(SQLiteDatabase db) {
        ...
        try {
            try {
                NodeList nodeList = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(this.m_context.getResources().getAssets().open("phonetype.xml")).getDocumentElement().getElementsByTagName("phoneType");
                ...
                Log.d(this.TAG, var16.getMessage());
            } catch (Throwable th) {
                throw th;
            }
    ...
    public VCardList queryContacterInfo(SQLiteDatabase db, String fullName, String phoneNum, int StorageType, String address) {
        HashSet phoneInfos;
        DbHelperPbap dbHelperPbap = this;
        ...
        Cursor cursor = null;
        if (gotName) {
            cursor = dbHelperPbap.queryContacterByFullName(sQLiteDatabase, fullName, StorageType, address);
        } else if (gotNumber) {
            cursor = dbHelperPbap.queryContacterByPhoneNum(sQLiteDatabase, phoneNum, StorageType, address);
        }
        List<VCardPack> vcardPacks = new ArrayList<>();
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                Cursor numCursor = dbHelperPbap.queryPhoneNumberByContentId(sQLiteDatabase, cursor.getString(cursor.getColumnIndex("_id")));
    ...
    private Cursor queryContacterByFullName(SQLiteDatabase db, String fullName, int StorageType, String address) {
        return db.rawQuery("select * from PhoneBookContent where FullName like ? and StorageType = ? and CellPhone_Address = ?", new String[]{fullName + "%", BuildConfig.FLAVOR + StorageType, address});
    }

    private Cursor queryContacterByPhoneNum(SQLiteDatabase db, String phoneNum, int StorageType, String address) {
        return db.rawQuery("select * from PhoneBookContent where _id in (select Content_ID from PhoneNumberDetail where Number like ?) and StorageType = ? and CellPhone_Address = ?", new String[]{"%" + phoneNum + "%", BuildConfig.FLAVOR + StorageType, address});
    }

    public Cursor queryContacterByPhoneNumAndAddress(SQLiteDatabase db, String phoneNum, String address) {
        return db.rawQuery("select FullName from PhoneBookContent where _id in (select Content_ID from PhoneNumberDetail where Number = ?) and CellPhone_Address = ?", new String[]{phoneNum, address});
    }
    ...
    public Cursor queryContacterByLikePhoneNumAndAddress(SQLiteDatabase db, String phoneNum, String address, int position) {
        ...
        return db.rawQuery("select a.FullName, b.Number from PhoneBookcontent a inner join phonenumberdetail b on a._id = b.Content_id where b.Number like ? and a.cellphone_address = ? order by b.Number;", new String[]{result, address});
    }
    ...
    public int deleteVcardInfo(android.database.sqlite.SQLiteDatabase r13, java.lang.String r14, int r15) {
        /*
            r12 = this;
            r0 = 0
            java.lang.String r1 = "_id, PhotoPath"
        ...
        L_0x0031:
            ...
            com.goodocom.bttek.util.normal.NfUtils.deleteFile(r6)
        L_0x0059:
        ...
        L_0x0074:
            ...
            return r0
        */
        throw new UnsupportedOperationException("Method not decompiled: com.goodocom.bttek.util.db.DbHelperPbap.deleteVcardInfo(android.database.sqlite.SQLiteDatabase, java.lang.String, int):int");
    }
    ...
    public void insertVcardInfo(SQLiteDatabase db, VCardPack vcardPack, int type) {
        ...
        if (vcardPack != null) {
            ...
            for (PhoneInfo phoneInfo : vcardPack.getPhoneNumbers()) {
                ...
                db.insert("PhoneNumberDetail", str, _cv);
            }
        }
    }
    ...
    public void insertCallHistoryInfo(SQLiteDatabase db, VCardPack vcardPack, int type) {
        ...
        if (vcardPack != null) {
            ...
            try {
                numName = Integer.valueOf(name).intValue();
                String str = this.TAG;
                Log.v(str, "numName : " + numName);
            } catch (NumberFormatException var9) {
                var9.printStackTrace();
            }
            if (numName < 0) {
                String str2 = this.TAG;
                Log.e(str2, "name is " + numName + " , automatically set it to \"Private Number\"");
                name = "Private Number";
            }
            String str3 = BuildConfig.FLAVOR;
            cv.put("FullName", name == null ? str3 : vcardPack.getFullName());
            cv.put("FirstName", vcardPack.getFirstName() == null ? str3 : vcardPack.getFirstName());
            cv.put("LastName", vcardPack.getLastName() == null ? str3 : vcardPack.getLastName());
            cv.put("CellPhone_Address", vcardPack.getCellPhone_Address() == null ? str3 : vcardPack.getCellPhone_Address());
            cv.put("StorageType", Integer.valueOf(type));
            for (PhoneInfo phoneInfo : vcardPack.getPhoneNumbers()) {
                cv.put(PHONE_TYPE, phoneInfo.getPhoneType() == null ? str3 : phoneInfo.getPhoneType());
                cv.put("PhoneNumber", phoneInfo.getPhoneNumber() == null ? str3 : phoneInfo.getPhoneNumber());
            }
            cv.put("HistoryDate", vcardPack.getHistoryDate() == null ? str3 : vcardPack.getHistoryDate());
            if (vcardPack.getHistoryTime() != null) {
                str3 = vcardPack.getHistoryTime();
            }
            cv.put("HistoryTime", str3);
    ...
    public void deleteContacterById(SQLiteDatabase db, int id) {
        db.delete("PhoneNumberDetail", "Content_ID=?", new String[]{BuildConfig.FLAVOR + id});
        db.delete(PHONEBOOK_CONTENT, "_id=?", new String[]{BuildConfig.FLAVOR + id});
    }

    public void deleteCallHistoryById(SQLiteDatabase db, int id) {
        db.delete(CALLHISTORY_CONTENT, "_id=?", new String[]{BuildConfig.FLAVOR + id});
    }
    ...
    public void deleteAllTableContent(SQLiteDatabase db) {
        String str = null;
        String[] strArr = null;
        db.delete("PhoneNumberDetail", str, strArr);
